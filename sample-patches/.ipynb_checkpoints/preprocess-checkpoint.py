...
class Preprocess:
    ...
    def preprocess_inference(self, patient_dict):
        ...
        for i in range(3):
            if dims[i] % patch_radius[i] == 0:
                ...
                pad_width.append(-1 * (int(np.ceil(dims[i] / patch_radius[i]) * patch_radius[i]) - dims[i]))
        ...
        return image_npy, dims, pad_dims, pad_width
    ...
    def run(self):
        for i in trange(len(self.df)):
            ...
            mask_onehot = np.empty((*dims, len(self.params['labels'])))
            ...
            patch_radius = [patch_dim // 2 for patch_dim in self.params['patch_size']]
            for j in range(self.params['samples_per_patient']):
                # Oversample foreground patches 85% of the time
                if np.random.uniform() <= 0.85:
                    # Pick a foreground point (i.e., any label that is not 0)
                    # Randomly pick a foreground class
                    label_idx = np.random.randint(1, len(self.params['labels']))
                    
                    
                    if label_index_ranges[label_idx + 1] > label_index_ranges[label_idx]:
                        point_idx = np.random.randint(label_index_ranges[label_idx], 
                                                      label_index_ranges[label_idx + 1])
                    else:
                        # Pick any foreground point
                        # This takes care of the case when a label is not present in a patient
                        point_idx = np.random.randint(label_index_ranges[1], 
                                                      label_index_ranges[-1])
                    point = label_points[..., point_idx]
                else:
                    # Pick a background point
                    label_idx = 0
                    point_idx = np.random.randint(label_index_ranges[label_idx], 
                                                  label_index_ranges[label_idx + 1])
                    point = label_points[..., point_idx]
                ...
                image_patch = image_npy[point[0] - patch_radius[0]:point[0] + patch_radius[0], 
                                        point[1] - patch_radius[1]:point[1] + patch_radius[1], 
                                        point[2] - patch_radius[2]:point[2] + patch_radius[2], 
                                        ...]
                mask_patch = mask_onehot[point[0] - patch_radius[0]:point[0] + patch_radius[0], 
                                         point[1] - patch_radius[1]:point[1] + patch_radius[1], 
                                         point[2] - patch_radius[2]:point[2] + patch_radius[2], 
                                         ...]
                
                feature = {'image': self.float_feature(image_patch.ravel()),
                           'mask': self.float_feature(mask_patch.ravel())}
